...
export default function WebSummary() {
  ...
  const handleSummarize = async () => {
    if (!url || !url.startsWith("http")) {
      setError("Please paste a valid full URL starting with http:// or https://");
      return;
    }
    ...
    try {
      const res = await API.post("/web/summarize", { url, language });
      setSummary(res.data.summary);
    } catch (err) {
      console.error("Web Summary Error:", err);
      if (err.response?.data?.message === "AI Quota Exceeded") {
        setError("AI Quota Exceeded. Please check your AI service billing details.");
      } else {
        setError(err.response?.data?.message || "Failed to fetch or summarize the webpage. Some sites block automated tools.");
      }
    } finally {
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 sm:p-12 border border-slate-200 dark:border-slate-800 shadow-sm mb-12 flex flex-col items-center transition-colors">
          ...
          <div className="w-full max-w-2xl flex flex-col sm:flex-row gap-4 justify-between items-center mb-6">
            ...
            <button
              ...
              className="w-full sm:w-auto bg-blue-600 text-white font-bold px-8 py-3 rounded-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-blue-600/20"
            >
              {isSummarizing ? (
                <>
          ...
          {isSummarizing && (
            <div className="w-full max-w-2xl text-left bg-white dark:bg-slate-900 rounded-2xl p-6 sm:p-12 border border-blue-100 dark:border-slate-800 shadow-sm transition-colors mb-8">
              <div className="flex items-center gap-3 mb-4">
                  <div className="w-5 h-5 border-2 border-primary border-t-transparent rounded-full animate-spin flex-shrink-0"></div>
                  <p className="text-sm font-bold text-primary dark:text-blue-300">Scraping & Summarizing...</p>
              </div>
        ...
        {summary && (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-500 transition-colors">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-start sm:items-center justify-between bg-slate-50/50 dark:bg-slate-800/50 flex-col sm:flex-row gap-4 transition-colors">
              <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            {/* Chat Interface */}
            <ChatBox contextText={summary} sourceName="Webpage" language={language} />
          </div>
        )}
      </div>
    </div>
